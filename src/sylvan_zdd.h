...
#define zdd_from_mtbdd(dd, domain) CALL(zdd_from_mtbdd, dd, domain)

/**
 * Convert a ZDD to an MTBDD.
 */
TASK_DECL_2(MTBDD, zdd_to_mtbdd, ZDD, ZDD);
#define zdd_to_mtbdd(dd, domain) CALL(zdd_to_mtbdd, dd, domain)
...
// #define zdd_set_to_mtbdd(orig) ... zdd_to_mtbdd(restrict1(orig), orig)
...
#define zdd_union_cube(set, variables, values, leaf) CALL(zdd_union_cube, set, variables, values, leaf)
...
#define zdd_extend_domain(dd, newvars, value) CALL(zdd_extend_domain, dd, newvars, value);
...
#define zdd_pathcount(dd) CALL(zdd_pathcount, dd)
...
#define zdd_ite(f, g, h, dom) CALL(zdd_ite, f, g, h, dom)
...
#define zdd_project(dd, domain) CALL(zdd_project, dd, domain)

/**
 * Compute \forall <vars>: <dd>.
 */
TASK_DECL_2(ZDD, zdd_forall, ZDD, ZDD);
#define zdd_forall(dd, vars) CALL(zdd_forall, dd, vars)
...
#define zdd_and_project(a, b, domain) CALL(zdd_and_project, a, b, domain)
...
#define zdd_enum(dd, dom, cb, context) CALL(zdd_enum, dd, dom, cb, context)

VOID_TASK_DECL_4(zdd_enum_seq, ZDD, ZDD, zdd_enum_cb, void*);
#define zdd_enum_seq(dd, dom, cb, context) CALL(zdd_enum_seq, dd, dom, cb, context)

LACE_TYPEDEF_CB(ZDD, zdd_collect_cb, void*, uint8_t*, size_t);
TASK_DECL_5(ZDD, zdd_collect, ZDD, ZDD, ZDD, zdd_collect_cb, void*);
#define zdd_collect(dd, dom, res_dom, cb, context) CALL(zdd_collect, dd, dom, res_dom, cb, context)
*/
...
#define zdd_visit_seq(...) CALL(zdd_visit_seq, __VA_ARGS__)

/**
 * Parallel visit operation
 */
VOID_TASK_DECL_4(zdd_visit_par, ZDD, zdd_visit_pre_cb, zdd_visit_post_cb, void*);
#define zdd_visit_par(...) CALL(zdd_visit_par, __VA_ARGS__)
...
#define zdd_writer_tobinary(file, dds, count) CALL(zdd_writer_tobinary, file, dds, count)
...
#define zdd_writer_totext(file, dds, count) CALL(zdd_writer_totext, file, dds, count)
...
#define zdd_writer_add(sl, dd) CALL(zdd_writer_add, sl, dd)
...
#define zdd_reader_frombinary(file, dds, count) CALL(zdd_reader_frombinary, file, dds, count)
...
#define zdd_reader_readbinary(file) CALL(zdd_reader_readbinary, file)
