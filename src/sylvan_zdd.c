 ...
 * zdd_gettype
 * zdd_getvalue
 * either zdd_getint64, zdd_getdouble, zdd_getnum, zdd_getdenom
 *     or an alternative way to get these
 */
...
VOID_TASK_IMPL_1(zdd_gc_mark_rec, ZDD, zdd)
{
    ...
    if (llmsset_mark(nodes, ZDD_GETINDEX(zdd)) == 0) return;

    // Check if not a leaft
    zddnode_t n = ZDD_GETNODE(zdd);
    if (zddnode_isleaf(n)) return;
...
VOID_TASK_2(zdd_refs_mark_r_par, ZDD*, arr, size_t, count)
{
    if (count < 32) {
        while (count) {
            zdd_gc_mark_rec(*arr);
        ...
        SPAWN(zdd_refs_mark_r_par, arr, count/2);
        CALL(zdd_refs_mark_r_par, arr+count/2, count-count/2);
        SYNC(zdd_refs_mark_r_par);
    }
}

VOID_TASK_2(zdd_refs_mark_s_par, Task**, arr, size_t, count)
{
    if (count > 0) {
        Task *t = *arr;
    ...
    if (count < 32) {
        while (count) {
            ...
            if (TASK_IS_COMPLETED(t)) {
                zdd_gc_mark_rec(*(ZDD*)TASK_RESULT(t));
            }
            arr++;
            count--;
        }
    } else {
        SPAWN(zdd_refs_mark_s_par, arr, count/2);
        CALL(zdd_refs_mark_s_par, arr+count/2, count-count/2);
        SYNC(zdd_refs_mark_s_par);
    }
}

/**
 * Mark all internal references (during garbage collection)
 */
VOID_TASK_0(zdd_refs_mark_task)
{
    LOCALIZE_THREAD_LOCAL(zdd_refs_key, zdd_refs_internal_t);
    SPAWN(zdd_refs_mark_r_par, zdd_refs_key->results, zdd_refs_key->r_count);
    CALL(zdd_refs_mark_s_par, zdd_refs_key->spawns, zdd_refs_key->s_count);
    SYNC(zdd_refs_mark_r_par);
}
...
TASK_IMPL_2(ZDD, zdd_clause_subsume, ZDD, a, ZDD, b)
{
    ...
    zdd_refs_spawn(SPAWN(zdd_clause_subsume, a_pv, b_pv));
    ZDD C = zdd_refs_push(zdd_clause_subsume(a_dc, b_dc));
    ZDD B = zdd_refs_push(zdd_refs_sync(SYNC(zdd_clause_subsume)));
    ZDD A = zdd_refs_push(zdd_refs_sync(SYNC(zdd_clause_subsume)));
    zdd_refs_spawn(SPAWN(zdd_clause_subsume, A, b_dc));
    ZDD E = zdd_refs_push(zdd_clause_subsume(B, b_dc));
    ZDD D = zdd_refs_push(zdd_refs_sync(SYNC(zdd_clause_subsume)));
    result = zdd_makenode(pv, C, E);
    result = zdd_makenode(nv, result, D);
...
TASK_IMPL_1(ZDD, zdd_clause_self_subsume, ZDD, set)
{
    ...
    const uint32_t set_var = zddnode_getvariable(set_node);
    const ZDD set_low = zddnode_low(set, set_node);
    const ZDD set_high = zddnode_high(set, set_node);
    if (set_low == set_high) return zdd_clause_self_subsume(set_low);
    ...
    zdd_refs_spawn(SPAWN(zdd_clause_self_subsume, set_low));
    ZDD high = zdd_refs_push(zdd_clause_self_subsume(set_high));
    const ZDD low = zdd_refs_push(zdd_refs_sync(SYNC(zdd_clause_self_subsume)));
    high = zdd_clause_subsume(high, low);
    zdd_refs_pop(2);
 ...
 * But also reduces things with empty clause to empty claus
 */
